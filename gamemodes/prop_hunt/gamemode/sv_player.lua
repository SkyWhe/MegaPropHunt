...
function PlayerMeta:CalculateSpeed()
	...
	if self:IsDisguised() then
		local mul = math.Clamp(self:GetNWFloat("disguiseVolume", 1) / 200, 0.9, 1.2)
		
		settings.walkSpeed = settings.walkSpeed * mul
		if(self:GetNWFloat("disguiseVolume", 1) < 10) then
			settings.walkSpeed = settings.walkSpeed * .75
		end
		if settings.runSpeed > settings.walkSpeed then
			settings.runSpeed = settings.walkSpeed
		end
		local mul2 = math.Clamp(self:GetNWFloat("disguiseVolume", 1) / 200, 0.90, 1.05)
		settings.jumpPower = settings.jumpPower * mul2
	end
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	...
	if IsValid(ent) then
		ply:CSpectate(OBS_MODE_ROAMING, ent)
	end
