...
function GM:EndRoundMapVote()
	...
	for k, map in pairs(self.MapList) do
		...
		function but:Paint(w, h)
			...
			draw.SimpleText(dname, "RobotoHUD-15", 128 + 20, 5, color_white, 0)
			...
			local fg2 = draw.GetFontHeight("RobotoHUD-15")
			if votes > 0 then
				draw.SimpleText(votes .. (votes > 1 and " votes" or " vote"), "RobotoHUD-15", 128 + 600, 5, color_white, 0)
			end
