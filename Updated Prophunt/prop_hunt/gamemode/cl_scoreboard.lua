...
local skull = Material("rip.png", "noclamp")
...
local function addPlayerItem(self, mlist, ply, pteam)
	...
	function but:Paint(w, h)
		...
		if IsValid(ply) && ply:IsPlayer() then
			...
			if !ply:Alive() then
				draw.ShadowText(ply:Nick().." -RIP", "RobotoHUD-L20", s, 0, col, 0)
			else
	...
	if IsValid(menu) then
		...
		function menu.Credits:Paint(w, h) 
			...
			draw.ShadowText(t, "RobotoHUD-25", 4, 0, Color(199, 49, 29), 0)
				...
				if(settings.Round_num == nil) then
				draw.ShadowText("Round: " .. 1 .. "/" .. "6", "RobotoHUD-25", 4 + tw + 400, h, Color(220, 220, 220), 0, 4)

				else
				draw.ShadowText("Round: " .. settings.Round_num .. "/" .. "6", "RobotoHUD-25", 4 + tw + 400, h, Color(220, 220, 220), 0, 4)
