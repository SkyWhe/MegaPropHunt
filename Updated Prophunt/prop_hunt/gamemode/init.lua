...
GM.HunterDamagePenalty = CreateConVar("ph_hunter_dmgpenalty", 5, bit.bor(FCVAR_NOTIFY), "Amount of damage a hunter should take for shooting an incorrect prop" )
...
function GM:Initialize()
	...
	self.RoundSettings.Round_num = 1
	self.RoundSettings.Round_max = 6
	self.DeathRagdolls = {}
...
function GM:InitPostEntityAndMapCleanup() 
	...
	for k, ent in pairs(ents.GetAll()) do
		...
		if IsValid(phys)then
			ent:SetNWInt("Mass", phys:GetMass())
		end
		...
		if ent:IsDisguisableAs() then
			...
			if IsValid(att) && att:IsPlayer() && att:Team() == 2 then
				...
				if bit.band(dmginfo:GetDamageType(), DMG_CRUSH) != DMG_CRUSH then					
					...
					tdmg:SetDamage(self.HunterDamagePenalty:GetInt())
...
function GM:ShowSpare1(ply)
	ply:ConCommand("ph_taunt_random")
